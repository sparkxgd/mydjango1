		      ...
		      ,{field: 'status' ,title:'状态', fixed: 'center',
		    	  templet:function(d){
			    	  if(d.status==0){
			    		  ...
			    		  return '<span style="color: green" >迟到</span>'
			    	  }
			    	  else if(d.status==2){
			    		  return '<span style="color: orange" >旷课</span>'
			    	  }
			    	  else if(d.status==3){
			    		  return '<span style="color: yellow" >事假</span>'
			    	  }
			    	  else {
			    		  return '<span style="color: red" >病假</span>'
			    	  }
			      } 
		      }
	  ...
	  if(layEvent === 'check'){
		 ...
		 //关闭当前提示	
		  ...
		  var index = layui.layer.open({
              title : "修改信息",
              type : 2,
              content : "openStu_studyEdit?id="+data.id,
              success : function(layero, index){
                  setTimeout(function(){
                      layui.layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                          tips: 3
